...
# find bubbles leaving the domain from ceiling
def find_bubbles_leaving_ceiling(before_adv, after_adv, ceiling_height, entrained_thresh):
    # before_adv: [x, y, z, St, entrained_thresh]
    # after_adv: [x, y, z, St, entrained_thresh]
    # ceiling_height: height of the ceiling
    # entrained_thresh: threshold for entrainment

    combined_info = np.concatenate([before_adv[:, :-2], after_adv], axis=1)
    # find bubbles that are below the ceiling before advecting and above the ceiling after advecting
    deposit_bubbles = combined_info[(before_adv[:, 2] < ceiling_height) & (after_adv[:, 2] > ceiling_height) 
                                    & (after_adv[:, -1] > entrained_thresh)]
    return deposit_bubbles
...
def advect_bubbles(bubbles_df_to_advect, t0, tf, plot_path = False, this_ax = None, color=None):
    ...
    return res_array # need to return deposited bubbles as well!!!
